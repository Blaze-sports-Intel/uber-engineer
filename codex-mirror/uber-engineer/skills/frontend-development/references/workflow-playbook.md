...
- **Audit accessibility against WCAG 2** → Accessibility review report mapped to WCAG SCs.
...
- **Author Storybook stories, MSW fixtures, and visual regression baselines** → Storybook story template with controls and a11y addon.
- **Separate UI state, server state, form state, and URL state with predictable patterns** → Visual regression plan with diff thresholds.
