# Blockchain Development — Workflow Playbook
...
- **Author Solidity or Anchor contracts with explicit access control.** → Contract spec: state, events, access roles, upgrade path.
- **Write deterministic test suites against forked mainnet state.** → Foundry/Hardhat test suite with coverage report, including fork tests against a pinned block.
- **Run static analysis (Slither, Mythril) before deploy.** → Static analysis report with each finding triaged.
- **Design wallet flows that don't ask users to sign opaque blobs.** → Wallet UX flow: human-readable confirmations + simulation preview.
- **Build event indexers for on-chain state into off-chain queries.** → Indexer schema mapped to contract events.
- **Apply reentrancy, integer overflow, and signature-replay guards by default.** → Mainnet deploy checklist with multisig sign-off, plus a security guard inventory (CEI pattern, ReentrancyGuard, EIP-712 nonces).
