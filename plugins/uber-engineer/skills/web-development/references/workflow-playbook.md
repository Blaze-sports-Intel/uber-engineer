...
- **Wire routing, layouts, error boundaries, and not-found at the framework boundary.** → Routing + layout architecture spec including 404, 500, and not-found behaviors.
- **Configure SEO: titles, descriptions, canonical, OpenGraph, Twitter Cards, JSON-LD.** → SEO config per route.
- **Optimize Core Web Vitals at the page level with measured budgets.** → Performance budget per route with measured baseline.
- **Deploy with preview URLs per PR and cache invalidation on release.** → Deploy pipeline with preview + production + rollback.
- **Add web-vitals + RUM + error reporting at the framework boundary.** → Observability dashboard: vitals, errors, traffic — plus an accessibility statement page wired into the app.
